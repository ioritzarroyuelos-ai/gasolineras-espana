#!/usr/bin/env node
// v1.38 — Descarga las farmacias de guardia de Burgos desde el portal central
// CGCOF (farmaciasguardia.farmaceuticos.com), id provincia=09.
//
// Burgos tiene ~25 zonas basicas de salud. Reusamos el helper compartido
// del CGCOF.
//
...
const CACHE_FILE = resolve(CACHE_DIR, 'burgos-geo.json')
const OUT_FILE = resolve(DATA_DIR, 'guardias-burgos.json')

const PROVINCIA_ID = '09'
const PROVINCIA_NOMBRE = 'Burgos'
const USER_AGENT = 'cercaya-guardias/1.38 (+https://webapp-3ft.pages.dev)'

// Bbox provincia Burgos (margen generoso; provincia interior grande).
const BBOX = { minLat: 41.5, maxLat: 43.3, minLng: -4.5, maxLng: -2.5 }
...
async function main() {
  ...
  console.log(`Descargando guardias Burgos (CGCOF id=09) — fecha ${fecha}...`)

  const { cookies, zonas, referer } = await obtenerSesionYZonas(PROVINCIA_ID, USER_AGENT)
  console.log(`  ${zonas.length} zonas detectadas`)
  if (zonas.length < 15) {
    throw new Error(`Solo ${zonas.length} zonas para Burgos. La estructura cambio?`)
  }
  ...
  if (farmaciasMap.size > 200) {
    throw new Error(`Sospechoso: ${farmaciasMap.size} farmacias. Max razonable ~100.`)
  }
