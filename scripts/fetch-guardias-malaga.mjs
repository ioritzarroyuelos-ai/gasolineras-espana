#!/usr/bin/env node
// v1.34 — Descarga las farmacias de guardia de Malaga desde el portal central
// CGCOF (farmaciasguardia.farmaceuticos.com), id provincia=29.
//
// El COF de Malaga (icofma.es) embebe el portal CGCOF en iframe — no expone
// API propia. Usamos el mismo helper compartido que Zaragoza y Badajoz.
//
...
const CACHE_FILE = resolve(CACHE_DIR, 'malaga-geo.json')
const OUT_FILE = resolve(DATA_DIR, 'guardias-malaga.json')

const PROVINCIA_ID = '29'
const PROVINCIA_NOMBRE = 'Málaga'
const USER_AGENT = 'cercaya-guardias/1.34 (+https://webapp-3ft.pages.dev)'

// Bbox provincia Malaga.
const BBOX = { minLat: 36.3, maxLat: 37.4, minLng: -5.6, maxLng: -3.7 }
...
async function main() {
  ...
  console.log(`Descargando guardias Malaga (CGCOF id=29) — fecha ${fecha}...`)

  const { cookies, zonas, referer } = await obtenerSesionYZonas(PROVINCIA_ID, USER_AGENT)
  console.log(`  ${zonas.length} zonas detectadas`)
  if (zonas.length < 50) {
    throw new Error(`Solo ${zonas.length} zonas para Malaga. La estructura cambio?`)
  }

  // Recolectar todos los IDs unicos de farmacia de todas las zonas.
  ...
  // En Malaga capital la "Localidad" trae "(29011) - SECTOR ...". El CP
  // 29001-29018 es Malaga capital. Para geocodear bien sustituimos el
  // sector por "Málaga" como municipio real y guardamos el sector en zona.
  for (const d of detalles) {
    const cpNum = parseInt(d.cp, 10)
    if (cpNum >= 29001 && cpNum <= 29018 && /^SECTOR\b/i.test(d.municipio)) {
      d.zona = d.municipio
      d.municipio = 'Málaga'
    } else if (!d.municipio || d.municipio === '') {
      d.zona = ''
    } else {
      d.zona = ''
    }
  ...
  // Geocodificar con cache. Para Malaga capital usamos "Direccion, CP Malaga"
  // que da mejor recall en Nominatim que la sintaxis con sectores.
  ...
  for (const d of detalles) {
    ...
    guardias.push([
      ...
      d.telefono,
      d.cp,
      d.tipoGuardia, // DIA / NOCHE / DIA / NOCHE combinacion
