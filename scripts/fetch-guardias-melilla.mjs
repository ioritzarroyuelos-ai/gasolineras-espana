#!/usr/bin/env node
// v1.37 — Descarga las farmacias de guardia de Melilla desde el portal central
// CGCOF (farmaciasguardia.farmaceuticos.com), id provincia=52.
//
// Melilla solo tiene 1 zona (52000001 - MELILLA) y unas pocas farmacias de
// guardia al dia (suelen ser 2-3). Reusamos el helper compartido de CGCOF.
//
...
const CACHE_FILE = resolve(CACHE_DIR, 'melilla-geo.json')
const OUT_FILE = resolve(DATA_DIR, 'guardias-melilla.json')

const PROVINCIA_ID = '52'
const PROVINCIA_NOMBRE = 'Melilla'
const USER_AGENT = 'cercaya-guardias/1.37 (+https://webapp-3ft.pages.dev)'

// Bbox Melilla (ciudad autonoma, area pequeña; margen generoso para cubrir
// posibles farmacias en periferia).
const BBOX = { minLat: 35.25, maxLat: 35.35, minLng: -2.99, maxLng: -2.91 }
...
async function main() {
  ...
  if (farmaciasMap.size > 50) {
    throw new Error(`Sospechoso: ${farmaciasMap.size} farmacias. Max razonable ~20.`)
  }
