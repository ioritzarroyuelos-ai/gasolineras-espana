#!/usr/bin/env node
// v1.38 — Descarga las farmacias de guardia de Salamanca desde el portal central
// CGCOF (farmaciasguardia.farmaceuticos.com), id provincia=37.
//
// Salamanca tiene ~17 zonas basicas de salud (Salamanca Ciudad, Alba de
// Tormes, Aldeadavila, Bejar, Calzada de Valdunciel - Ledesma, Ciudad
// Rodrigo, Guijuelo, La Fuente de San Esteban, Penaranda, Vitigudino...).
// Reusamos el helper compartido del CGCOF.
//
...
const CACHE_FILE = resolve(CACHE_DIR, 'salamanca-geo.json')
const OUT_FILE = resolve(DATA_DIR, 'guardias-salamanca.json')

const PROVINCIA_ID = '37'
const PROVINCIA_NOMBRE = 'Salamanca'
const USER_AGENT = 'cercaya-guardias/1.38 (+https://webapp-3ft.pages.dev)'

// Bbox provincia Salamanca.
const BBOX = { minLat: 40.0, maxLat: 41.4, minLng: -7.1, maxLng: -5.1 }
...
async function main() {
  ...
  console.log(`Descargando guardias Salamanca (CGCOF id=37) — fecha ${fecha}...`)

  const { cookies, zonas, referer } = await obtenerSesionYZonas(PROVINCIA_ID, USER_AGENT)
  console.log(`  ${zonas.length} zonas detectadas`)
  if (zonas.length < 10) {
    throw new Error(`Solo ${zonas.length} zonas para Salamanca. La estructura cambio?`)
  }
  ...
  if (farmaciasMap.size > 200) {
    throw new Error(`Sospechoso: ${farmaciasMap.size} farmacias. Max razonable ~80.`)
  }
